<style stylus>
    ...
    .progress{
        position: absolute; left: 0; top: 0; z-index: 160; width: 100%; }
    .progress .bar{
        transition: all 1s ease;
    }
</style>
<template>
    <div class="progress" v-show="isShow" :class="['progress-' + state, cls]" v-if="isProgress">
        <div class="bar tran" :style="{ width: width + '%' }"></div>
    </div>
    <div v-if="isPromise" v-show="isShow" class="loading-wrapper" :class="'loading-wrapper-' + state" :style="{ height: height + 'px' }">
        <div class="loading-promise loading" :class="[cls, isSuccess && 'loading-promise-success', isError && 'loading-promise-error' ]">
            <div class="inner"></div>
        </div>
    </div>
</template>
